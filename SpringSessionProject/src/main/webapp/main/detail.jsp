...
<body>
	...
	<script type="text/babel">
		class Detail extends React.Component{
			...
			/*componentDidMount()
			{
				axios.get("http://localhost:8081/web/main/detail_data.do",{
					params:{
						mno:this.state.mno 
					}
				}).then((result)=>{
					...
					<tr>
						...
						<td class="text-right">	
							<c:if test="${sessionScope.id==rvo.id }">
								<span class="btn btn-xs btn-danger spans_update" data-no="${rvo.no }">수정</span>
								<a href="reply_delete.do?no=${rvo.no}&mno=${mno}" class="btn btn-xs btn-success">삭제</a>
							</c:if>
								<span class="btn btn-xs btn-info spans" data-no="${rvo.no}">댓글</span>
						</td>
					</tr>
					<tr>
						<td colspan="2">
				...
				<tr>
					<td>
						<form method="post" action="reply_insert.do">
							<textarea rows="3" cols="125" name="msg" style="float:left"></textarea>
							<input type="hidden" name="mno" value="${mno}">
							<button class="btn btn-sm btn-success" style="float:left; height:65px">글쓰기</button>
						</form>
					</td>
				</tr>
			</table>
